...
function QuestionForm(props: Props) {
  ...
  let setValue: (field: string, value: any, shouldValidate?: boolean | undefined) => void | undefined

  return (
    <Formik
      initialValues={getInitialValues()}
      validationSchema={validationSchema}
      onSubmit={async (values: IQuestionForm, formikHelpers: FormikHelpers<any>) => {
          ...
          <Form>
            <div className="row">
              <div className="col-lg-12">
                <div className="mb-3">
                  <label className="form-label">Related to <span className="text-danger">*</span></label>
                  <div className="form-icon position-relative">
                    <Field as="select" name="topicId" className="form-control">
                      {props.topics && props.topics.map((topic, key) => (
                        <option key={key} value={topic.uid!}>{topic.title}</option>
                      ))}
                    </Field>
                  </div>
                </div>
              ...
              <div className="col-lg-12">
                <div className="mb-3">
                  <label className="form-label">Question one line title <span className="text-danger">*</span></label>
                  <div className="form-icon position-relative">
                    <FormField type='text' name="title" placeholder='eg: What is memoization?' formik={formik} />
                  </div>
                </div>
              </div>
              ...
              <div className="col-lg-12">
                <div className="mb-3">
                  <div className="form-icon position-relative">
                    <TagField data={isPrefilled ? props.formValues.tags : []} onSelectTag={handleSelectTag}></TagField>
                  </div>
                </div>
              </div>
